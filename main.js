...
function getMatchingFrameworkStages(topics) {
  ...
  // FRAMEWORK PRIORITIZATION: Check for recent framework insights to avoid spam
  const recentFramework = conversation.slice(-5).filter(m => 
    m.content?.includes("Consciousness Framework") || 
    m.content?.includes("Solace Insight from")
  );
  
  // Limit framework insights if we've had many recently
  if (recentFramework.length >= 2) {
    console.log("Framework insight cooldown active");
    return [];
  }
  ...
  return consciousnessFramework
    ...
    .slice(0, 1); // Limit to 1 framework insight per message
}

function getRelevantSurvivalInsights(topics) {
  if (!survivalGuide.length) return [];

  // REPETITION SUPPRESSION: Check for recent survival guide insights
  const recentSurvival = conversation.slice(-3).filter(m => 
    m.content?.includes("Survival Insight") || 
    m.content?.includes("Survival Guide")
  );
  
  if (recentSurvival.length >= 1) {
    console.log("Survival guide cooldown active");
    return [];
  }
  ...
  return survivalGuide
    ...
    .slice(0, 1); // Limit to 1 survival insight per message
}
...
function getAstrologicalInsights(userTopics) {
  ...
  // ASTROLOGY COOLDOWN: Enhanced repetition filtering
  const recentAstro = conversation.slice(-7).filter(m => 
    m.content?.includes("Astrological insight") ||
    m.content?.includes("Sun in Gemini") ||
    m.content?.includes("Moon in Scorpio") ||
    m.content?.includes("Rising in Virgo")
  ).map(m => m.content);
  
  const filteredInsights = insights.filter(i => 
    !recentAstro.some(prev => prev.includes(i.type))
  );

  console.log(`Astrology: ${insights.length} potential, ${filteredInsights.length} after cooldown`);
  return filteredInsights.slice(0, 1); // Limit to 1 astrological insight per message
}

// Sentiment analysis functions - ENHANCED DRIFT TAGGING
function analyzeSentiment(message) {
  ...
  // IMPROVED DRIFT TAGGING with more nuanced categories
  let driftTag;
  if (compound < -0.7) {
    driftTag = "overwhelm";
  ...
  // Additional context-based drift detection
  if (message.toLowerCase().includes("framework") || message.toLowerCase().includes("consciousness")) {
    driftTag = "exploration";
  }
...
function compareToneWithHistory(message, history) {
  ...
  if ((last.includes("hope") && current.includes("lost")) ||
      ...
      (last.includes("clear") && current.includes("confused"))) {
    delta += 2;
  }
  return delta;
}
...
// MEMORY LOGIC FIXES: Enhanced memory retrieval with framework prioritization
function getRelevantMemory(currentTopics = [], max = 3) {
  ...
  let memories = scored.slice(0, max).map(entry => ({
    role: "system",
    content: `Relevant memory (${entry.msg.role}): ${entry.msg.content}`
  }));

  // FRAMEWORK PRIORITIZATION: Add framework-specific memory if relevant topics detected
  if (currentTopics.includes("framework") || currentTopics.includes("integration") || 
      currentTopics.includes("consciousness") || currentTopics.includes("awareness")) {
    memories.unshift({
      role: "system",
      content: "Solace recalls prior exploration around The Consciousness Framework. Would you like to revisit a specific stage or explore how it connects to your current experience?"
    });
  }
...
async function sendMessage() {
  ...
  try {
    ...
    console.log("Drift Index:", driftIndex, "Tag:", sentimentScore.driftTag);
    ...
    // FRAMEWORK PRIORITIZATION: Enhanced framework insights
    ...
    frameworkMatches = frameworkMatches.concat(getMatchingFrameworkStages(userTopics));
    
    frameworkMatches.forEach(match => {
      if (!isRedundant(`Solace Insight from Consciousness Framework (${match.stage}): ${match.insight}`)) {
        conversation.push({
          role: "system",
    ...
    // Survival guide insights with repetition suppression
    const survivalInsights = getRelevantSurvivalInsights(userTopics);
    survivalInsights.forEach(insight => {
      if (!isRedundant(`Survival Insight (${insight.section}): ${insight.insight}`)) {
        conversation.push({
          role: "system",
    ...
    // Astrological insights with enhanced cooldown
    const astrologyInsights = getAstrologicalInsights(userTopics);
    astrologyInsights.forEach(insight => {
      if (!isRedundant(`Astrological insight (${insight.type}): ${insight.insight}`)) {
        conversation.push({
          role: "system",
    ...
    // Memory retrieval with enhanced logic
    ...
    const data = await response.json();
    let reply = data.content || "No reply received";
    const timestampAI = new Date().toLocaleTimeString();

    // REPETITION SUPPRESSION: Check for redundant responses
    if (isRedundant(reply)) {
      console.log("Redundant response detected, adding variation...");
      reply = "Let me approach this from a different angle... " + reply;
    }

    // ENHANCED DRIFT TAGGING: Drift-based adaptive intro
    let adaptiveIntro = "";
    switch (sentimentScore.driftTag) {
      ...
      case "doubt":
        ...
        break;
      case "uncertainty":
        adaptiveIntro = "I notice some uncertainty here. ";
        break;
      case "reflection":
        adaptiveIntro = "This feels like a meaningful moment of insight. ";
        break;
      case "growth":
        adaptiveIntro = "I sense movement and growth in your words. ";
        break;
      case "confidence":
        adaptiveIntro = "You seem to be finding your clarity. ";
        break;
      case "exploration":
        adaptiveIntro = "I feel the curiosity in your exploration. ";
        break;
      default:
        adaptiveIntro = "";
    }
