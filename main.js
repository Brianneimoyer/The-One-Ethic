...
function getAstrologicalInsights(userTopics) {
  ...
  // Filter out recent astrological insights to avoid repetition
...
function getRelevantMemory(currentTopics = [], max = 3) {
  ...
  // Add framework-specific memory if relevant topics detected
...
async function sendMessage() {
  ...
  try {
    ...
    if (isRedundant(reply)) {
      console.log("Redundant response detected, regenerating...");
